/*
 * Using the periodic timer mode. Tested using one of the LEDs on the TIVA Launchpad
 * main.c
...
int main(void) {
    ...
    /*not using interrupts for now*/
    //step 7: Set the TnEN bit in the GPTMCTL register to enable the timer and start counting.
    TIMER0->CTL |= (1u << 0);

    while(1){

    	if(TIMER0->RIS & 0x00000001){     //this means the counter has timed out
    		TIMER0->ICR |= (1u << 0);
